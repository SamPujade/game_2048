...
graphical_grid=[]
for i in range(16):
    frame=Frame(root,bg=TILES_BG_COLOR[grid[i//n][i%n]],height=90,width=160)
    graphical_grid.append(frame)
for j in range(16):
    Label(graphical_grid[j], text=grid_with_theme(grid)[j//n][j%n],fg=TILES_FG_COLOR[grid[i//n][i%n]]).pack(padx=16, pady=9)
    graphical_grid[j].grid(row=j//n,column=j%n)
root.mainloop()
